...
const CelebrationPage = () => {
  ...
  useEffect(() => {
    if (submitted) {
      ...
      const audio = new Audio("https://www.myinstants.com/media/sounds/yay.mp3");
      audio.play().catch(() => {});
    ...
    <div className="relative w-full h-screen overflow-hidden flex flex-col items-center justify-center bg-gradient-to-br from-pink-100 via-yellow-50 to-blue-100">
      ...
      {!submitted ? (
        <div className="z-20 flex flex-col items-center text-center p-4">
          <img src={payment} alt="Scan to pay" className="w-64 rounded shadow mb-4" />
          <h2 className="text-xl text-gray-700 font-semibold">After payment, enter your name</h2>
        ...
        <>
          ...
          <div className="z-30 text-center mt-72 fade-in">
            <h1 className="text-4xl font-bold text-pink-600 mb-2">Thank you, {name}!</h1>
            <p className="text-xl text-gray-700">You just made someone's day better 💕</p>
          </div>
        </>
      )}
    </div>
